...
## 작업 흐름 — SG Match 방식 (조사 → 검토 → 승인 → 적용·검증 → 검수 → 배포)

> AI(Claude / Antigravity)는 코드를 고치거나 새로 만들 때 아래 방식을 따른다. SG Match 프로젝트와 동일한 운영 원칙이다.

### 3대 철칙
1. 메모·인수인계·AI의 "완료" 보고를 곧이곧대로 믿지 않는다. 항상 실제 코드 / git log / 실제 결과로 직접 확인한다.
2. 코드를 직접 보기 전엔 아무것도 단정하지 않는다. 추정으로 진행하지 않는다.
3. 한 번에 다 하지 않는다. 위험한 것(데이터·연결 로직)부터, 단계를 끊어서 한다.

### 단계 (기존 코드를 고칠 때)
1. **[조사 — read-only]** 고치기 전에 관련 파일을 수정 없이(read-only) 먼저 본다. "이 파일의 이 부분을 그대로 보여줘, 수정하지 마"로 현재 코드를 확인하고, 그림이 완전해질 때까지 반복한다(다른 파일의 함수도 추적).
2. **[검토 + diff 제시]** 실제 코드를 면밀히 검토하고, 무엇을·어느 파일 몇 번째 줄에·어떻게 바꿀지 diff(변경 전/후)를 먼저 보여준다. 왜 안전한지 초보자 눈높이로 설명한다. 사용자 승인 전엔 적용하지 않는다.
3. **[적용 — 검증, push 금지]** 승인 후 적용 프롬프트를 실행한다. 이 프롬프트는 항상: ① 지정한 부분만 고치고 나머지는 절대 건드리지 말 것 ② 수정 후 검증 실행 — 문법 검사(HTML/JS = 태그·문법, Python = py_compile) → git status(그 파일만 바뀌었는지) → git diff(지시한 곳만 바뀌었는지) ③ 결과를 보고하되 git add/commit/push는 하지 말 것.
4. **[검수]** 검증 결과를 한 줄씩 대조한다: 바뀐 파일이 의도한 것 하나뿐인지, diff가 지시대로인지, 보안(escapeHtml 등)·기존 동작이 유지됐는지.
5. **[배포 — 별도 승인]** 검수 통과 후 별도 단계로: git add <그 파일만>(git add . 절대 금지) → git commit -m "영문 메시지" → git push origin main.
6. **[배포 확인 + 실주행 테스트]** push 후 배포가 실제로 됐는지 확인하고, 직접 동작시켜 의도대로 되는지 확인한다. 이게 통과해야 "완료" 확정.

### 새로 만들 때
위 1번(조사)이 "기준이 될 기존 코드 읽기"로 바뀐다. 예: pages/index.html을 읽어 디자인 토큰·nav·푸터를 파악 → 기준 코드(또는 빌드 프롬프트) 작성 → 3~6단계 동일.

### 안티그래비티 감시
Antigravity가 승인 없이 바로 적용하거나 지시 안 한 부분까지 건드리면, 멈추고 사용자에게 알린다.

### 가드레일 보강
- git add . 금지. 항상 바꾼 파일 이름을 직접 지정한다. push 전 git status·git diff로 의도한 파일·줄만 바뀌었는지 확인한다.
- 파일 삭제·덮어쓰기, 돈 드는 작업(유료 API·결제·외부 배포)은 실행 전 사용자에게 확인받는다.
